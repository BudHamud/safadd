...
export function TransactionEditView({ tx, initialData, categories, userId, onSave, onClose }: Props) {
  ...
  const analyzeAsset = async (asset: ImagePicker.ImagePickerAsset) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : t('order.scan_error');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await onSave({
        ...
        goalType,
        details: nextDetails,
        cardDigits: paymentMethod === 'tarjeta' ? cardDigits || null : null,
      });
      if (result === false) {
        haptic.error();
    ...
    <Modal visible transparent statusBarTranslucent onRequestClose={onClose} animationType="fade">
      <View style={[styles.overlay, { backgroundColor: 'rgba(0,0,0,0.75)' }]}>
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
          ...
          <View style={styles.field}>
            <Text style={[styles.label, { color: C.textMuted }]}>{t('order.title_label').toUpperCase()}</Text>
            <TextInput
              style={[styles.input, { backgroundColor: C.surface, borderColor: C.border, color: C.textMain }]}
              value={description}
              onChangeText={setDescription}
              placeholder={t('mobile.order.description_placeholder')}
              placeholderTextColor={C.textMuted}
            />
          </View>
        ...
        >
          <View style={[styles.sheetOverlay, { backgroundColor: 'rgba(0,0,0,0.72)' }]}> 
...
const styles = StyleSheet.create({
  ...
  amountRow: { flexDirection: 'row', gap: 0, alignItems: 'stretch', borderWidth: 1, borderRadius: 2, overflow: 'hidden' },
  currencyPill: { width: 86, borderRightWidth: 1, alignItems: 'center', justifyContent: 'center', gap: 2 },
  currencyPillButton: { paddingVertical: 6 },
  currencyCode: { fontSize: 10, fontWeight: FontWeight.black, letterSpacing: 1 },
  currencySymbol: { fontSize: FontSize.lg, fontWeight: FontWeight.black },
  input: { fontSize: FontSize.base, paddingHorizontal: Spacing.md, paddingVertical: Spacing.md, borderRadius: 2, borderWidth: 1 },
  amountInput: { flex: 1, fontSize: FontSize.xl, fontWeight: FontWeight.black, textAlign: 'center', borderWidth: 0, borderRadius: 0 },
  paymentRow: { flexDirection: 'row', gap: Spacing.sm },
  paymentBtn: { flex: 1, borderWidth: 1, borderRadius: 2, paddingVertical: 8, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 },
  paymentBtnText: { fontSize: 10, fontWeight: FontWeight.black, letterSpacing: 0.4 },
  cardDigitsWrap: { borderWidth: 1, borderRadius: 2, paddingHorizontal: 12, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', gap: 10 },
  cardDigitsMask: { flex: 1 },
  cardDigitsMaskText: { fontSize: 13, fontWeight: FontWeight.black, letterSpacing: 2.4 },
  cardDigitsInput: { width: 54, fontSize: 18, fontWeight: FontWeight.black, letterSpacing: 2, textAlign: 'right' },
