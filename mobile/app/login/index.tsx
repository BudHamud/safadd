  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'login') {
        ...
        const parsedMonthlyGoal = wantsGoal ? Number(monthlyGoal.replace(',', '.')) : 0;
        const { error } = await signUp(
          email.trim().toLowerCase(),
          password,
          fullName.trim() || undefined,
          Number.isFinite(parsedMonthlyGoal) && parsedMonthlyGoal >= 0 ? parsedMonthlyGoal : 0,
        );
        if (error) {
          const msg = error.message.toLowerCase();
          ...
          <View style={[styles.authBox, { backgroundColor: C.surface, borderColor: C.border }]}>
            ...
            <View style={[styles.authLogoBar, { borderBottomColor: C.borderDim }]}>
              <View style={styles.authLogoWrap}>
                <AnimatedLogoMark size={28} animate={loading} showHalo={loading} />
              </View>
